...
const Checkout = () => {
  ...
  const handledelete = (cartids) => {
   ...
   console.log("cartddddddd",cartids)
    const jwtToken = localStorage.getItem("JWTtoken");
    const customHeaders = {
      authorization: `${jwtToken}`, // Replace 'YourAuthToken' with your actual authorization token
      "Content-Type": "application/json", // Specify the content type if needed
    ...
    //     // Product deletion was successful, now delete the cart item
    //     return axios.delete(`http://localhost:3300/product/remove_product?id=${id}`,{
    //       headers: customHeaders,})}
    //    else {
            ...
            {products.map((product, index) => (
              <div className="carts" key={index}>
                ...
                {categories.map((categoryItems) => {
                  if (product.c_id === categoryItems.cat_id) {
                    return (
                      <h5 class="cart-category_names" key={categoryItems.cat_id} >
               ...
               {carts.map((cartes, cartIndexs) => {
                if (cartIndexs === index) {
                  return (
                    ...
                    <svg onClick={() => handledelete(cartes.cart_id)} xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" className="bi bi-trash" viewBox="0 0 16 16">
